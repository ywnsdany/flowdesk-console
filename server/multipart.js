...
// Buffers the entire body up to maxBytes, then splits by boundary.
// Returns parts: [{ name, filename?, contentType?, data: Buffer }, ...].
...
function splitParts(buf, boundary) {
  ...
  let start = indexOf(buf, dash, 0);
  if (start === -1) return parts;
  start += dash.length;
  while (start < buf.length) {
    // After boundary either CRLF (more parts) or "--" (end).
    if (buf[start] === 0x2d && buf[start + 1] === 0x2d) break; // --
    if (buf[start] === 0x0d && buf[start + 1] === 0x0a) start += 2;
    const headersEnd = indexOf(buf, Buffer.from('\r\n\r\n'), start);
    if (headersEnd === -1) break;
    const rawHeaders = buf.slice(start, headersEnd).toString('utf8');
    const dataStart = headersEnd + 4;
    const nextBoundary = indexOf(buf, Buffer.concat([crlf, dash]), dataStart);
