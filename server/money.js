...
export function computeClosing(input, openingBalanceHalalas, appsEnabled = true) {
  // Foodics is the source of truth for total. Network total is what the network device reads.
  ...
  // Per-app breakdown — only if enabled. Otherwise apps_total = 0.
  ...
  const expected = openingBalanceHalalas + cashSales;
  ...
  return {
    ...
    opening_balance_halalas: openingBalanceHalalas,
